...
public:
    ...
    GuiTextureBoxInspector(const String& windowCaption, const shared_ptr<Texture>& texture, Texture::Visualization& settings, const shared_ptr<GuiWindow>& parentWindow, GApp* app) :
        ...
        const Vector2 screenBounds((float)parentWindow->window()->width(), (float)parentWindow->window()->height());
...
void GuiTextureBox::rawSave() {
    if (notNull(m_app)) {
        const String& savePath = m_app->screenCapture()->getNextFilenameBase() + "_" + FilePath::makeLegalFilename(m_texture->name()) + ((m_texture->format()->redBits == 8) ? ".png" : ".exr");
